...
public class Users extends JTabbedPane implements MouseListener {
    ...
    public Users() {
        testManagerComponents.setUsers(this);

        addMouseListener(this);

        Config config = Config.getInstance();
        if (config.isOpenedFolderPathActive()) {
            File activeFolder = new File(config.getConfigDetails().getTestManagerConfigDetails().getOpenedFolderPath());
    ...
    public void mouseReleased(MouseEvent e) {
        if (getTabCount() > 1 && e.getModifiersEx() == MouseEvent.META_DOWN_MASK && e.getClickCount() == 1) {
            ...
            deleteMenuItem.addActionListener(e1 -> {
                ...
                removeTabAt(index);
                GuiUtils.refreshComponent(testManagerComponents.getUsers());
            });
